...
4.  **MeshManga REST ID Chapter Number Bypass**: Modified the `extract_chapter_number(title, url)` function inside `core/database.py` to bypass URL-based regex parsing entirely when the URL contains `meshmanga.com` or `appswat.com`. This prevents REST database IDs in the URL path (e.g., `/chapters/1751659/`) from polluting the chapter float parser, forcing the parser to fall back immediately to extracting the correct chapter float (e.g., `2.0`) from the Arabic title, restoring correct chronological sorting and multi-source deduplication.
...
1.  **[COMPLETED] Multi-Source Integration Phase 2 (MeshManga)**: MeshManga REST Ingestion is 100% active, running on Live Pagination with Async Background Ingestion. Resolved the catalog pagination duplication issue by removing the read-first cache lookup on the `/api/v1/manga/catalog` route, relying on live pagination with background `BackgroundTasks` database serialization. Documented the Vercel lifespan database connection circuit-breaker check (`check_db_online()`) and Android Team X masquerading name injection on the frontend. The staging environment is currently isolated with Olympus paused, preparing for multi-source merging tomorrow.
