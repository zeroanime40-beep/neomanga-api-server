...
async def scrape_meshmanga_details(manga_url: str) -> dict:
    """
    ...
    async with httpx.AsyncClient(headers=HEADERS, timeout=5.0, follow_redirects=True) as client:
        ...
        detail_res = await client.get(details_url)
        ...
        while page_url:
            ch_res = await client.get(page_url)
...
async def scrape_meshmanga_pages(chapter_url: str) -> list[str]:
    """
